...
Current product version: `0.12.4`. The web app verifies the bridge before opening the console, and the sidebar shows both the web and loaded bridge versions so analysts can confirm that Chrome is using the expected build.
...
The packaged bridge currently trusts the exact HTTPS origins `https://laptop-1.tail029be8.ts.net` and `https://laptop-1.tail029be8.ts.net:8443`. If the console moves to another origin, update the manifest and both extension runtime allowlists, then rebuild and reinstall the bridge. Reload the console tab once after installing an unpacked extension if Chrome has not injected its content script yet.
